...
const querySelector = require('../query/query-go').querySelector;
...
exports.info = function(page) {
    return new Promise((resolve, reject) => {
        (async function() {
            ...
            info.formatoEmissao      = await selector.queryInnerText(page, querySelector.info.formatoEmissao);
            info.formatoEmissao      = info.formatoEmissao ? info.formatoEmissao.replace(/\s+/g, ' ').trim() : null;
            info.digitoVerificador   = await selector.queryInnerText(page, querySelector.info.digitoVerificador);
            info.digitoVerificador   = info.digitoVerificador ? info.digitoVerificador.replace(/\s+/g, ' ').trim() : null;
            info.identifacaoAmbiente = await selector.queryInnerText(page, querySelector.info.identifacaoAmbiente);
            info.identifacaoAmbiente = info.identifacaoAmbiente ? info.identifacaoAmbiente.replace(/\s+/g, ' ').trim() : null;
            info.finalidade          = await selector.queryInnerText(page, querySelector.info.finalidade);
            info.finalidade          = info.finalidade ? info.finalidade.replace(/\s+/g, ' ').trim() : null;
            info.processo            = await selector.queryInnerText(page, querySelector.info.processo);
            info.processo            = info.processo ? info.processo.replace(/\s+/g, ' ').trim() : null;
            info.versao              = await selector.queryInnerText(page, querySelector.info.versao);
            info.versao              = info.versao ? info.versao.replace(/\s+/g, ' ').trim() : null;
            info.entradaContigencia  = await selector.queryInnerText(page, querySelector.info.entradaContigencia);
            info.entradaContigencia  = info.entradaContigencia ? info.entradaContigencia.replace(/\s+/g, ' ').trim() : null;
            info.justificativa       = await selector.queryInnerText(page, querySelector.info.justificativa);
            info.justificativa       = info.justificativa ? info.justificativa.replace(/\s+/g, ' ').trim() : null;
